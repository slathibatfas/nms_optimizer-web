...
const MessageSpinner: React.FC<MessageSpinnerProps> = ({ isInset = true, isVisible, initialMessage, showRandomMessages = false }) => {
  ...
  return (
    ...
    <div className={containerClasses.trim()}>
      ...
      {initialMessage && <Text className="pt-4 text-xl font-bold shadow-sm sm:text-2xl messageSpinner__header">{initialMessage}</Text>}
      <Text
        className={`text-sm font-semibold text-center shadow-sm sm:text-normal messageSpinner__random ${
          displayRandomMessage ? "messageSpinner__random--visible" : "messageSpinner__random--hidden"
        }`}
      >
