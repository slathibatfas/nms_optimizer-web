...
const TechTreeRowComponent: React.FC<TechTreeRowProps> = ({
	...
	const imagePath = techImage
		? `${baseImagePath}${techImage}`
		: fallbackImage;
						...
						>
							...
							>
								...
								<Accordion.Content className="AccordionContent">
									{modules
										.filter(
											(module) => module.type === 'reward'
										)
										.map((module) => (
											<div
												key={module.id}
												className="flex items-center gap-2 AccordionContentText"
											>
												<Checkbox
													className="CheckboxRoot"
													variant="soft"
													id={module.id}
													checked={currentCheckedModules.includes(
														module.id
													)}
													onClick={() =>
